<div id="#thevoyage" class="voyage hide-for-small">
	<div class="trajectory"></div>
	<div class="shuttle-fog"></div>
	<div class="sphere-5 sphere exosphere">
		<div class="digit-1 digit"><div class="inner">v.</div></div>
		<div class="digit-0 digit"><div class="inner">exo</div></div>
	</div>
	<div class="sphere-4 sphere thermosphere">
		<div class="digit-1 digit"><div class="inner">iv.</div></div>
		<div class="digit-0 digit"><div class="inner">thermos</div></div>
	</div>
	<div class="sphere-3 sphere mesosphere">
		<div class="digit-1 digit"><div class="inner">iii.</div></div>
		<div class="digit-0 digit"><div class="inner">mesos</div></div>
	</div>
	<div class="sphere-2 sphere stratosphere">
		<div class="digit-1 digit"><div class="inner">ii.</div></div>
		<div class="digit-0 digit"><div class="inner">stratos</div></div>
	</div>
	<div class="sphere-1 sphere troposphere">
		<div class="digit-1 digit"><div class="inner">i.</div></div>
		<div class="digit-0 digit"><div class="inner">tropos</div></div>
	</div>
	<div class="shuttle">
		<div class="rotator"></div>
	</div>
	<div class="wing-container">
		<div class="leftwing"></div>
		<div class="rightwing"></div>
		<div class="leftjet"></div>
		<div class="rightjet"></div>
	</div>
	<div class="follower-2"></div>
</div>	